/**
 * @fileoverview Application entry point — ShoppingCartService
 * @module index.ts
...
app.get("/health", (_req, res) => {
  res.json({ service: "ShoppingCartService", status: "ok", ts: new Date().toISOString() });
});

app.use("/cart", apiLimiter, routes);
...
async function waitForDb(maxAttempts = 20, delayMs = 3000): Promise<void> {
  ...
  const port = parseInt(process.env.DB_PORT || "5435");
...
async function bootstrap(): Promise<void> {
  logger.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  logger.info(`  ShoppingCartService — startup`);
  logger.info(`  DB    → ${process.env.DB_HOST || "localhost"}:${process.env.DB_PORT || "5435"}/${process.env.DB_NAME || "shopping_cart"}`);
  logger.info(`  Kafka → ${process.env.KAFKA_BROKERS || "localhost:9092"}`);
  logger.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  try {
    await waitForDb();
    await waitForKafka();
    app.listen(PORT, () => logger.info(`\n🚀  ShoppingCartService ready → http://localhost:${PORT}\n`));
  } catch (err) {
