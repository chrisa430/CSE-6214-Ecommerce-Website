...
app.use(cors({ origin: process.env.CORS_ORIGIN || "http://localhost:5173", credentials: true })); // restricts browser origins

// applies a 20-request/15-minute window on all /auth routes
...
app.use(express.json({ limit: "16kb" })); // parses bodies with a 16 KB cap

app.get("/health", (_req, res) => {
  res.json({ service: "AuthnAuthzService", status: "ok", ts: new Date().toISOString() });
});
...
async function bootstrap(): Promise<void> {
  ...
  const dbName = process.env.DB_NAME || "authn_authz";
  const kafka  = process.env.KAFKA_BROKERS || "localhost:9092";

  logger.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  logger.info("  AuthnAuthzService — startup");
  ...
  try {
    ...
    await waitForKafka();
    app.listen(PORT, () =>
      logger.info(`\n🚀  AuthnAuthzService ready → http://localhost:${PORT}\n`)
    );
  } catch (err) {
