...
// Public endpoint — returns active products for a given seller (used by BuyerSellerPage)
...
router.get("/products/:id/reviews", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Enrich each review with the buyer's name from the account DB (non-fatal)
    ...
    if (reviews.length > 0) {
      ...
      try {
        ...
        logger.warn("Account pool lookup failed for reviews (non-fatal)", accountErr);
        for (const r of reviews) {
