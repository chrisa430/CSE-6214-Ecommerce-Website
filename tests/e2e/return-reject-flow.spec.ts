...
test("buyer requests return and seller rejects it", async ({ page }) => {
  const productName = `Reject Return Ball ${Date.now()}`;
  const startingQty = 5;
  const price = 30;
  const returnReason = "Wrong item received";
  ...
  expect(pageText.toLowerCase()).toMatch(/rejected|declined|pending/);
});